...
| C18 | **T6** — capability recovers the ai-native density loss | scripted 93%→83% on ai-native; live Haiku 100%→100% (+17% recovery) | 2 seeds × 2 profiles | density floor is a weak-reader artifact | `tests/test_theory_t6.py`, `results/t6-capability-density/` | `pytest tests/test_theory_t6.py` |
| C19 | **T10** — LIVE-scribe capture closes the chain (salient chatty facts, Haiku, 2 seeds): a live model doing its own capture loses nothing vs oracle capture and keeps the lead over summarization | capture 100% (60/60), 0 noise ops; end-to-end +26.7% vs summary; +6.7% vs mech-rope (parity) | 60 | [+13.3%, +40.0%] vs summary; [−5.0%, +18.3%] vs mech (parity) | `results/t10-scribe/result-cmd.json` | `JROPE_BENCH_BUDGET_USD=1 python research/exp_t10_scribe.py --scribe cmd --cmd "claude -p --model haiku"` |
| C20 | **T11** — unsalient capture boundary (values only inside tool blobs; Haiku, 2 seeds, small-n directional) | unhinted capture 81% (13/16), −25% vs oracle [−50%, −6%]; +1 hint rule → 88%, net +12.5% [−12.5%, +37.5%] (parity); summary 0% on blob facts | 16 | see cell | `results/t11-unsalient/` | `JROPE_BENCH_BUDGET_USD=1 python research/exp_t11_unsalient.py --scribe cmd [--hint]` |
